...
class MSSpeech():
	...
	errors = {
		1007: "the voice name is not valid"
	}
	...
	def __init__(self):
		...
		try: self._voices_list = loop.run_until_complete(self.get_voices_list())
		except: self._voices_list = []
	...
	async def get_voice(self)->dict:
		voices:List[Dict] = await self.get_voices_list()
		for voice in voices:
			if voice["Name"].strip() == voiceName.strip():
				return voice
		return {}
	...
	async def synthesize(self, text:str, filename_or_buffer:Any)-> None:
		...
		async with aiohttp.ClientSession(headers = self.headers) as session:
			...
			await ws.send_str(
				...
				"""
<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='en-US'><voice  name='{voiceName}'><prosody pitch='+0Hz' rate ='+0%' volume='+0%'> {text}</prosody></voice></speak>
			""".strip().format(text = text, voiceName = self.voiceName)).decode("UTF8")
			)
			f:Any = None
			while True:
				msg = await ws.receive()
				if (f is not None and msg.type == aiohttp.WSMsgType.text and 'json' in msg.data) or (msg.type == aiohttp.WSMsgType.closed or msg.type == aiohttp.WSMsgType.error):
					await ws.close()
					break

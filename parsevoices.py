...
for tr in trs:
	...
	status = tds[4].split(",", 1)[0].strip()
	locale = tds[1].strip()
	name = tds[3].replace(locale+"-", "").strip()
	json_voices_list.append({
		"Name":f"Microsoft Server Speech Text to Speech Voice ({locale}, {name})",
		"ShortName":f"{tds[3].strip()}",
		"Gender":f"{tds[2].strip()}",
		"Locale":locale,
		"FriendlyName":f"{name.replace('Neural', '').strip()} - {tds[0].strip()}",
